...
const Newproduct = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const fetchData = await fetch(`${process.env.REACT_APP_SERVER_DOMAIN}/uploadProuct`, {
      method: "POST",
      headers: {
  ...
  return (
    <div className='p-4'>
      <form className='m-auto w-full max-w-md shadow-sm flex flex-col p-3 bg-white' onSubmit={handleSubmit}>
        ...
        <label htmlFor='image'>Image
          <div className='h-40 w-full bg-slate-200 my-3 rounded flex items-center justify-center cursor-pointer'>
            {
              data.image ? <img src={data.image} className='h-full' alt='productImage' /> : <span className='text-5xl'><BsCloudUpload /></span>
